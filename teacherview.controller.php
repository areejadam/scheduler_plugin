...
require_sesskey();
/*function send_sms_mess ($student){
    ...
    $numbers = $student ; // A single number or a comma-seperated list of numbers
    $message = "Sorry , the doctor has canceled the appointment ";
    // 612 chars or less
    ...
    case 'revokeall': {
        ...
        foreach ($slot->get_appointments() as $app) {
            ...
            foreach ($oldstudents as $oldstudent) {
                # ___________________ send notification to student __________________ #
                ...
                #____________________send SMS message to student _____________________#
                
                include_once($CFG->dirroot.'/mod/scheduler/classes/task/send_mess.php');
                $student= $DB->get_fieldset_sql('SELECT phone2 FROM mdl_user WHERE id = ?', array($oldstudent));
                $message = "sorry the appointment has ben canceled .";
                #send_sms_mess($student[0]);
            
                echo '<script type="text/javascript">alert("  sms send succussfully to  : '.$student[0].'")</script>';

                send_sms_mess::send_sms($student[0],$message);
            }
